...
class BinaryTreeTest {
    ...
    private fun testRemove1(create: () -> CheckableSortedSet<Int>) {
        ...
        list.add(40)
        val binarySet = create()
        assertFalse(binarySet.remove(39))
        for (element in list) {
            binarySet += element
        }
        val originalHeight = binarySet.height()
        val oldSize = binarySet.size
        assertTrue(binarySet.remove(45))
        assertEquals(oldSize - 1, binarySet.size)
        println("Removing 45 from $list")
        for (element in list) {
            val inn = element != 45
            assertEquals(
                inn, element in binarySet,
        ...
        assertTrue(
            binarySet.height() <= originalHeight,
            "After removal of 45 from $list binary tree height increased"
        )
    }
    ...
    private fun testIteratorRemove1(create: () -> CheckableSortedSet<Int>) {
        ...
        assertEquals(
            0, counter,
            "Iterator.remove() of 45 from $list changed iterator position: " +
                    "we've traversed a total of ${binarySet.size - counter} elements instead of ${binarySet.size}"
        )
        println()
        assertEquals<SortedSet<*>>(treeSet, binarySet, "After removal of 45 from $list")
        assertEquals(treeSet.size, binarySet.size, "Size is incorrect after removal of 45 from $list")
        for (element in list) {
            val inn = element != 45
            assertEquals(
                inn, element in binarySet,
